...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  try {
    ...
    const s3Key    = `jobs/${jobId}/${attachId}/${body.filename}`;
    const now      = new Date().toISOString();
    const username = (event.requestContext.authorizer?.claims?.['cognito:username'] as string) ?? 'unknown';

    // Generate presigned URL — client uploads directly to S3 (5 min expiry)
    ...
    // Record the attachment metadata in DynamoDB
