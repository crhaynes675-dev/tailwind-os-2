  ...
  return (
    <div className="mb-6 flex items-end justify-between gap-3">
      <div>
        ...
        <p className="mt-1.5 text-sm text-muted">{count} live job{count === 1 ? '' : 's'} · drag to advance · click for detail</p>
      </div>
      <button
        onClick={onReload}
        disabled={loading}
      ...
      {STATUS_PIPELINE.map((status) => {
        ...
        return (
          <div key={status} className="glass relative overflow-hidden rounded-2xl p-4">
            ...
            <div className="relative mt-2 text-[2.3rem] font-bold leading-none tracking-tight" style={{ color: meta.color }}>{count}</div>
          </div>
        );
      })}
    </div>
...
function JobCard({ job, onDragStart, onDragEnd, onClick, dragging }: { job: Job; onDragStart: () => void; onDragEnd: () => void; onClick: () => void; dragging: boolean }) {
  ...
  return (
    <div
      ...
      onDragEnd={onDragEnd}
      onClick={onClick}
      className={`cursor-pointer rounded-xl border border-white/5 bg-white/[0.03] p-3 transition hover:bg-white/[0.07] ${dragging ? 'opacity-40' : ''}`}
    >
...
function PipelineBoard({ jobs, onRequestMove, onOpen }: { jobs: Job[]; onRequestMove: (job: Job, to: JobStatus) => void; onOpen: (id: string) => void }) {
    ...
    <div className="grid gap-3.5" style={{ gridTemplateColumns: `repeat(${STATUS_PIPELINE.length}, minmax(190px, 1fr))` }}>
      {STATUS_PIPELINE.map((status) => {
        ...
        return (
          <div
            key={status}
            onDragOver={(e) => { if (dragJob) { e.preventDefault(); setOverStatus(status); } }}
            onDragLeave={() => setOverStatus((s) => (s === status ? null : s))}
            onDrop={(e) => {
              e.preventDefault();
              if (dragJob && dragJob.status !== status) onRequestMove(dragJob, status);
              setDragId(null);
              setOverStatus(null);
            }}
          ...
          >
            ...
            <div className="mt-auto border-t border-white/5 px-3.5 py-2 text-[0.55rem] text-faint">{meta.owner} · {meta.output}</div>
          </div>
        );
      })}
    </div>
  ...
  return (
    <div className="fixed bottom-6 left-1/2 z-50 -translate-x-1/2">
      <div className={`glass flex items-center gap-3 rounded-full px-5 py-2.5 text-sm ${toast.kind === 'err' ? 'text-[#f4607a]' : 'text-text'}`}>
        <span>{toast.msg}</span>
        {toast.undo && <button onClick={() => { toast.undo!(); onClose(); }} className="font-semibold text-accent hover:underline">Undo</button>}
...
export default function Dashboard() {
  const { jobs, loading, error, reload, updateJob, updateStatus, select } = useJobsCtx();
  ...
  const commit = useCallback(
    (job: Job, to: JobStatus, patch: Partial<Job>) => {
      const from = job.status;
      updateJob(job.id, patch)
        .then(() => setToast({ kind: 'ok', msg: `${job.workOrder} → ${STATUS_META[to].short}`, undo: () => updateStatus(job.id, from).catch(() => {}) }))
    ...
    <>
      ...
      {error ? (
        <div className="glass rounded-2xl border-[#f4607a]/30 p-6 text-center">
          ...
          <button onClick={reload} className="mt-4 rounded-lg bg-white/5 px-4 py-2 text-xs font-semibold text-accent hover:bg-white/10">Try again</button>
        </div>
        ...
        <>
          ...
          <div className="overflow-x-auto pb-2">
            <PipelineBoard jobs={jobs} onRequestMove={(job, to) => setPending({ job, to })} onOpen={select} />
          </div>
        </>
      )}
      ...
      {pending && (
        <TransitionModal
          ...
          onConfirm={(patch) => { commit(pending.job, pending.to, patch); setPending(null); }}
        />
      )}
