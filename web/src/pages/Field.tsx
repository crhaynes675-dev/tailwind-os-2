...
function fieldStep(job: Job): { label: 'En Route' | 'On Site' | 'Complete'; patch: Partial<Job> } | null {
...
function JobCard({ job, selected, onSelect, onStep, onPhoto, busy, uploading }: {
  ...
  onStep: (job: Job, patch: Partial<Job>) => void; onPhoto: (job: Job, f: File) => void;
  ...
  return (
    <div onClick={() => onSelect(job.id)}
      ...
      {(job.scheduledDate || job.onSiteAt) && (
        <div className="mt-1 text-[0.64rem] text-faint">
          {job.scheduledDate || ''}{job.onSiteAt ? ' · on site' : job.enrouteAt ? ' · en route' : ''}
        </div>
      )}
      ...
      <div className="mt-2 grid grid-cols-2 gap-2">
        ...
        <label onClick={(e) => e.stopPropagation()} className="cursor-pointer rounded-lg border border-glass bg-white/5 py-2 text-center text-xs font-semibold text-accent active:scale-[0.99]">
          {uploading ? 'Uploading…' : '📷 Photo'}
          <input type="file" accept="image/*" capture="environment" className="hidden"
            onChange={(e) => { const f = e.target.files?.[0]; if (f) onPhoto(job, f); e.target.value = ''; }} />
        </label>
      </div>
      ...
      {st ? (
        ...
        <div className="mt-2 rounded-lg bg-white/5 py-2 text-center text-[0.7rem] font-semibold text-completed">Completed — awaiting review</div>
      )}
    </div>
  );
}
...
export default function Field() {
  ...
  async function onStep(job: Job, patch: Partial<Job>) {
    setBusyId(job.id);
    try { await updateJob(job.id, patch); } finally { setBusyId(null); }
  }
  async function onPhoto(job: Job, file: File) {
    setUploadingId(job.id);
    try { await uploadAttachment(job.id, file); } finally { setUploadingId(null); }
  }

  const cardProps = (job: Job) => ({
    job, selected: selectedId === job.id, onSelect: setSelectedId, onStep, onPhoto,
    busy: busyId === job.id, uploading: uploadingId === job.id,
  });
    ...
    <div className="flex flex-col lg:h-[calc(100vh-9.5rem)]">
      ...
      <div className="grid flex-1 grid-cols-1 gap-4 lg:grid-cols-[380px_1fr] lg:overflow-hidden">
          ...
          {loading && myJobs.length === 0 ? (
            ...
            <>
              <div>
                ...
                {today.length === 0 ? (
                  <div className="text-[0.74rem] text-faint">Nothing scheduled for today.</div>
                ) : (
        ...
        <div className="min-h-[320px]">
          <FieldMap jobs={myJobs} onSelect={setSelectedId} focusId={selectedId || undefined} height="h-80 lg:h-full" />
        </div>
      </div>
    </div>
  );
