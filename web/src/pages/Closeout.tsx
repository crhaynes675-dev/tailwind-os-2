...
// Workflow 07 — the actionable closeout steps (drop the entry status + final "Closed" result).
const STEPS = WORKFLOWS.find((w) => w.id === '07')!.steps.slice(1, -1).map((s) => s.step);

export default function Closeout() {
  const { jobs, loading, select, updateJob } = useJobsCtx();
  const cl = useChecklist('closeout');
  const [pending, setPending] = useState<Job | null>(null);

  const queue = useMemo(() => jobs.filter((j) => j.status === 'Final Walkthrough Ready'), [jobs]);
  const meta = STATUS_META['Final Walkthrough Ready'];

  return (
    <>
      <div className="mb-6">
        <div className="mb-1 text-[0.62rem] font-semibold uppercase tracking-[0.25em] text-accent">Module · Workflow 07</div>
        <h1 className="bg-gradient-to-r from-[#22d3ee] to-[#7c6cff] bg-clip-text text-[2rem] font-bold leading-none tracking-tight text-transparent">Closeout</h1>
        <p className="mt-1.5 text-sm text-muted">Final walkthrough and customer approval. Closing a job triggers billing.</p>
      </div>

      {loading && jobs.length === 0 ? (
        <div className="glass grid place-items-center rounded-2xl py-24 text-sm text-muted">Loading…</div>
      ) : queue.length === 0 ? (
        <div className="glass grid place-items-center rounded-2xl py-20 text-center text-sm text-muted">No jobs ready for closeout.</div>
      ) : (
            ...
            return (
              <div key={j.id} className="glass flex flex-col rounded-2xl p-4">
                ...
                <button
                  ...
                  className="mt-4 rounded-lg bg-gradient-to-br from-[#22d3ee] to-[#34d39a] px-4 py-2 text-xs font-semibold text-[#04121a] transition hover:brightness-105"
                >
                  Close job →
                </button>
              </div>
